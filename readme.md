...
* **Control Protocol:** Uses MAVLink instead of IBUS.
* **Communication:** Operates over Wi-Fi (UDP) instead of a direct UART RC receiver connection. Requires a Raspberry Pi Pico **W**.
* **Control Method:** Designed for control via QGroundControl (or similar MAVLink GCS) running on a networked computer, replacing the need for a dedicated FlySky RC transmitter for direct flight control.
* **Arming/Disarming:** Managed via MAVLink commands sent from the GCS (specifically `MAV_CMD_COMPONENT_ARM_DISARM`), replacing the physical transmitter switch (Channel 5) used in the original version.
* **Required Libraries:** Adds dependencies on `mavlink.py`, `mavcrc.py`, and utilizes standard MicroPython libraries `socket` and `network`. `ibus.py` is no longer required.
* **Configuration:** Requires setting the IP address of the GCS computer (`QGC_COMPUTER_IP`) and potentially adjusting UDP ports within `main.py`.
* **Telemetry:** Sends standard MAVLink telemetry messages (Heartbeat, Attitude, etc.) to the GCS.
* **Safety:** Includes a basic MAVLink RC signal loss timeout for automatic disarming.

**Implications:**

* The setup and flight procedures described in the original articles and the sections below **no longer directly apply** to this MAVLink version. You will need to configure QGroundControl to connect to the Pico W via UDP.
* Hardware requirements now mandate a **Pico W** and a **Wi-Fi network**. The FlySky receiver/transmitter are not used for direct control in this configuration.
* PID tuning might need adjustments due to differences in control signal processing.
...
## Repository Contents
- The complete source code for the *Scout Flight Controller* can be found in [the `src` folder](./src/). *(Note: This folder now contains the MAVLink version).*
...
*(Note: This section refers to the original IBUS version. See the MAVLink update section above for current usage.)*
...
*(Note: These steps are for the ORIGINAL IBUS VERSION and DO NOT APPLY to the current MAVLink version. See the MAVLink update section above and code comments for MAVLink/QGroundControl setup.)*
